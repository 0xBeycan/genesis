...
- Follow your lane's loop (`workflow.md`) — in every lane the failing test
  precedes the code.
- Open an ADR the moment a decision rejects a live alternative (`AGENTS.md` §3);
  give every locked decision a `decisions.md` line.
...
2. **Rotate `progress.md` when it grows:** past ~10 entries **or ~120 lines**,
   move all but the newest 5 to `docs/memory/progress-archive.md` (same
   format, append at top).
...
6. **Keep the thin wrappers pure pointers** (`CLAUDE.md`, `README.md`,
