...
- **`ideate`** — diverge before you converge: for decisions with real design
  freedom (product angle, name, design direction, architecture fork), fix the
  constraints, generate 3–5 genuinely different options, converge to one,
  record the rejects. Opens the option space that `ponytail` then minimizes.
...
- **Claude Code** — symlink each skill folder into `.claude/skills/`
  (`ln -s ../../skills/<name> .claude/skills/<name>`), and the
  `UserPromptSubmit` hook in `.claude/settings.json` injects the always-on
  skill (`ponytail`) into every prompt. **Ships pre-wired** in the skeleton
...
> (and an ADR if it changes how the project is built). A skill earns its slot
> only when its absence produces a **named, recurring failure** — descriptions
> load every session, so the list is not free.
